...
void 
voice_init(Voice* v, const sample_t s_rate, Roland* r) {
    v->srate = s_rate;
...
void 
voice_init_all(Voice** v, sample_t s_rate, Roland* r){
        ...
        , const sample_t last_note)
{
    ...
    filter_reset(f1,filter1->cutoff,filter1->resonance,1.0
                , filter1->bandwidth);
    
    filter_init(f2, filter2->filter_type);
    filter_reset(f2,filter2->cutoff,filter2->resonance,1.0
                , filter2->bandwidth);    
    
    adsr_init(&v->env[0], *(p[V_EN1_ATT]), *(p[V_EN1_DEC])
            , *(p[V_EN1_SUS]), *(p[V_EN1_REL]), v->srate);
    v->env[0].loop = *(p[V_EN1_TRG]);
    adsr_init(&v->env[1], *(p[V_EN2_ATT]), *(p[V_EN2_DEC])
            , *(p[V_EN2_SUS]), *(p[V_EN2_REL]), v->srate);
    v->env[1].loop = *(p[V_EN2_TRG]);
    adsr_init(&v->env[2], *(p[V_EN3_ATT]), *(p[V_EN3_DEC])
            , *(p[V_EN3_SUS]), *(p[V_EN3_REL]), v->srate);
    v->env[2].loop = *(p[V_EN3_TRG]);
    adsr_init(&v->env[3], *(p[V_EN4_ATT]), *(p[V_EN4_DEC])
            , *(p[V_EN4_SUS]), *(p[V_EN4_REL]), v->srate);
...
void
voice_update(Voice** const voices, GlobalParms* gbl, Filter* const filter1
            , Filter* const filter2
            ...
            , const uint32_t len)
{
    ...
    for (unsigned int i = NUM_VOICES; i-- ; ) {
                ...
                if (unison>1) {
                    if (osc->uni_type) 
                    {
                        ...
                        if (j!=0) 
                        {
                            if (j % 2 == 1) 
                                unison_shift = 
                                 - pow(((j-unison/2.0)/unison * det2),2.0);
                            else 
                                unison_shift = 
                                   pow((((j-1)-unison/2.0)/unison * det2),2.0);
                        }
                    } 
                    else 
                    {
        ...
        osc_update(v->sub_osc, center_amp[0]
                , midi2freq(center_note[0]-12)); 
