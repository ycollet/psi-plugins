...
static inline void osc_phase(OscWave* const osc, sample_t _phase);

static inline void osc_update(OscWave* const osc, sample_t _amp, sample_t _freq);

// load the wave tables

static inline void osc_filltables(float** const table, const sample_t srate, const sample_t len);

static inline void blosc_make_tables(const sample_t s_rate, const sample_t len);

static inline sample_t osc_tic_freq(OscWave* const osc, sample_t amp, sample_t freq); 
...
static inline void osc_init(OscWave* const osc, enum osc_type _type, enum waveshape _shape, sample_t _srate,
                            long _tablen, sample_t _amp, sample_t _freq, sample_t _offset, sample_t _initialPhase,
                            const float** const table) {
  osc->shape  = _shape;
  osc->type   = _type;
  osc->srate  = _srate;
...
static inline void osc_phase(OscWave* const osc, sample_t _offset) {
...
static inline sample_t func_stair(int sample_num, sample_t tablen) {
...
static inline sample_t osc_tic_freq(OscWave *osc, sample_t amp, sample_t freq) {
  if (freq==0) return 0;
  sample_t out = 0;
  if ((osc->type == BLIT)) {
...
static inline sample_t osc_tic(OscWave *osc) {
  sample_t out;
