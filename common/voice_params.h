...
static inline float 
voice_param_value(int port, const double value, const int sync);

static inline float 
voice_param_display(int port, double value
                 , char* str_val
                 , char* desc
                 , const int sync);
...
static inline float voice_param_display(int port, const double value
                                      , char* str_val, char* desc
                                      , const int sync)
{
    if ((port < 0) || (port > V_PARAMS )) {
        printf("Port error: %d\n", port);
    ...
    else 
    {
        ...
        {
            snprintf( s, 128
                , parameters[port].format, 
                  parameters[port].display_values[f_round(val)]);
        } 
        else 
        {
            ...
            if (port == V_DLY_TML || port == V_DLY_TMR) 
            {
                ...
                snprintf( s, sizeof(delay_values[idx].description)
                , "%s", 
                  delay_values[idx].description);
            } 
            else 
            {
                idx = f_round((V_TEMPO-1) * value 
                        / (parameters[port].max - parameters[port].min));
                snprintf( s, sizeof(tempo_values[idx].description)
                , "%s", 
                  tempo_values[idx].description);
            }
