...
static void filter_alloc(Filter* f, sample_t srate);

static void filter_free(Filter* f);

static void filter_init(Filter* f, int ftype);

static void filter_reset(Filter* f, sample_t cutoff, sample_t resonance, sample_t amp, sample_t bandwith);

static sample_t filter_run(Filter* f, sample_t in);

static sample_t filter_run_moog1(Moog1* m, sample_t in, uint8_t filter_type);

static sample_t filter_run_moog2(Moog2* m, sample_t in, uint8_t filter_type);
...
static void filter_reset(Filter *f, sample_t cutoff, sample_t resonance,
                         sample_t amp, sample_t bandwith) {
  switch (f->filter_type)
    {
    case FILTER_EQ12:
      eq_set_params(&f->bq, f->cutoff, f->amp, f->bandwidth, f->sample_rate);
    case FILTER_LS:
      ls_set_params(&f->bq, f->cutoff, f->amp, 1.5, f->sample_rate);
      break;
    case FILTER_HS:
      hs_set_params(&f->bq, f->cutoff, f->amp, 1.5, f->sample_rate);
      break;
    ...
    case FILTER_LP24:
      ...
      if ( FILTER_4POLE == 2 ) {
        ...
        f->m2.c2            = f->m2.c * 2.0;
        f->m2.cgfbr         = 1.0 / (1.0 + f->m2.fb * f->m2.cg * f->m2.cg * f->m2.cg * f->m2.cg);
        }
    ...
    case FILTER_BP24:
      ls_filt_setup(&f->ls_bq,  LS_F_TYPE_BP, cutoff, resonance, f->sample_rate);
      ls_filt_setup(&f->ls_bq2, LS_F_TYPE_BP, cutoff, resonance, f->sample_rate);
      break;
            
    case FILTER_HP24:
      ls_filt_setup(&f->ls_bq,  LS_F_TYPE_HP, cutoff, resonance, f->sample_rate);
      ls_filt_setup(&f->ls_bq2, LS_F_TYPE_HP, cutoff, resonance, f->sample_rate);
      break;
...
static sample_t filter_run(Filter* f, sample_t in) {
  ...
  if ( (f->filter_type == FILTER_LP12 ) || (f->filter_type == FILTER_BP12 )
