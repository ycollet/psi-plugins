...
synth_init(SWSynth* const s, const float* const params[], const sample_t s_rate)
{
    ...
    osc_init(&s->lfo[0], LFO, 0, s_rate, 
                s->table_len, 1.0, 0.0, 0.0
                , 0.0, (const float** const) s->table);
    osc_init(&s->lfo[1], LFO, 0, s_rate,
                s->table_len, 1.0, 0.0, 0.0
                , 0.0, (const float** const) s->table);
    osc_init(&s->lfo[2], LFO, 0, s_rate,
                s->table_len, 1.0, 0.0, 0.0
                , 0.0, (const float** const) s->table);
...
void synth_frame(SWSynth* const s, uint32_t len)
{
    ...
    eq_set_params(&s->eq_filters[0], 100.0, *(s->params[V_LOW])
                , PEAK_BW, s->fs);
    eq_set_params(&s->eq_filters[1], 1000.0, *(s->params[V_MID])
                , PEAK_BW,  s->fs);
    hs_set_params(&s->eq_filters[2], 10000.0, *(s->params[V_HGH])
                , SHELF_SLOPE,  s->fs);
    ...
    if ( *(s->params[V_FL3_TYP]) != s->filter[2].filter_type )
    {
        ...
        filter_reset(&s->filter[2], s->filter[2].cutoff, 
                     s->filter[2].resonance,1.0, s->filter[2].bandwidth);    
    }
    ...
    if ( *(s->params[V_FL4_TYP]) != s->filter[3].filter_type )
    {
        ...
        filter_reset(&s->filter[3], s->filter[3].cutoff, 
                     s->filter[3].resonance,1.0, s->filter[3].bandwidth);    
    }
    ...
    voice_update(s->voices, &s->gbl, &s->filter[0], &s->filter[1]
                , (const float * const*) s->params, len); 
            ...
            , sample_t* const right)
{
    ...
    f3->cutoff          = mod_cutoff(s, &s->mod_cutoff3
                                    , *(s->params[V_FL3_CUT]));
    
    f4->cutoff          = mod_cutoff(s, &s->mod_cutoff4
                                    , *(s->params[V_FL4_CUT]));
    if (s->filter_input[2])
    {
        switch (s->filter_input[2]) {
            ...
            case FI_OSC2:
                ...
                break;
            case FI_OSC1OSC2:
                filt1_out += filter_run(f3, note2 * COS(M_PI*osc_mix/2)
                           + note1 * SIN(M_PI*osc_mix/2));
                break;
            case FI_FL1:
                // won't happen
                break;
    ...
    if (s->filter_input[3])
    {
        switch (s->filter_input[3]) {
            ...
            case FI_OSC2:
                ...
                break;
            case FI_OSC1OSC2:
                filt2_out += filter_run(f4, note2 * COS(M_PI*osc_mix/2)
                           + note1 * SIN(M_PI*osc_mix/2));
                break;
            case FI_FL1:
